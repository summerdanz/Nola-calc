...
def getResults():
    ...
    conviction_date2 = datetime.strptime(conviction_date[0:10], '%Y-%m-%d')
    arrest_date2 = datetime.strptime(arrest_date[0:10], '%Y-%m-%d')
    pretrialdetention = abs((conviction_date2 - arrest_date2))
    ...
    deltastring = str(sentencedeltadays)
    deltasplit = deltastring.split()
    a = deltasplit[0].split('.')[0]
    sentencedays = int(a[0])

    if eligibleGT == True:
        #ADD BELOW BACK IN IF NECESSARY
        # GTreleasedate = conviction_date + ((sentencedays) * GTmultiplier)
        GTreleasedate = conviction_date2 + ((sentencedeltadays) * GTmultiplier)
        ...
        GTdatestring = "Not eligible"

    ##Last possible release date
    #ADD BACK IN IF NECESSARY
    # lastreleasedate1 = int(conviction_date) + sentencedays
    lastreleasedate1 = conviction_date2 + sentencedeltadays
    lastdatestring= (str(lastreleasedate1)[:10])
    # goodTimeReleaseDate = GTdatestring
    goodTimeReleaseDate = "08-18-1988"
